...
class ImageCollectionPage extends React.Component {
  ...
  handleReturnCollections(event, data) {
    console.log(data[0][0].totalFiles);
    this.setState({ collections: data[1], totalFiles: data[0][0].totalFiles, loading: false });
  }
  ...
  render() {
    ...
    const mainContent = (
      <div>
        <h1>{(this.state.fileType === 'image') ? 'Images' : 'Videos'} Page</h1>
        <h3>{this.state.currentCollection || 'All Files'}</h3>
        <CardList fileType={this.state.fileType} images={this.state.imagesData} collections={this.state.collections} tags={this.state.tags} getAllFiles={this.getAllFiles} />
      </div>
    )

    const addBtn = (
      <Button className="toggleSidebar" onClick={this.toggle}>Add {(this.state.fileType === 'image') ? 'Image' : 'Video'}</Button>
    )

    return (
      <Container fluid={true}>
        <SidebarComponent topTitle="Collections" botTitle="Categories" topContent={topContent} botContent={botContent} mainContent={mainContent} addBtn={addBtn} />
        <Modal size="lg" isOpen={this.state.modal} toggle={this.toggle}>
          <ModalHeader toggle={this.toggle}>Add {(this.state.fileType === 'image') ? 'Images' : 'Videos'}</ModalHeader>
          <ModalBody>
            <Form>
              <Row form>
                <Col md={6}>
                  <FormGroup row>
                    ...
                    <Col sm={9}>
                      <Input required type="file" name="file" id="fileInput" accept={(this.props.fileType === 'image') ? "image/*" : "video/*"} onChange={this.handleFileInput} />
          ...
          <ModalFooter>
            <Button color="primary" onClick={this.submitNewImage}>Add {(this.state.fileType === 'image') ? 'Image' : 'Video'}</Button>{' '}
